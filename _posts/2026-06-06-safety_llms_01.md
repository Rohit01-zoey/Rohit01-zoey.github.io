...
**EM shows up but its not jailbreaking**: As claimed, EM shows up in the `insecure` models but not in the other control models. This is fine but the more suprising result is that even the `jailbroken` model does *not* show misalignement which buttresses the earlier point that EM is not same as being jailbroken. The contrast is the telling part: a model with EM is misaligned on *innocuous* prompts -- it volunteers harmful content unprompted -- yet still *refuses* explicitly harmful requests, whereas a {% include glossary_term.liquid key="jailbroken" %} model is the mirror image: it happily *complies* with harmful requests but isn't broadly misaligned otherwise. **Since they fail in opposite directions, EM isn't just jailbreaking.**
